...
Resolve each cluster's profile name first with the `resolve_cluster_agent(project_id, cluster_name, location)` tool, and assign a card only to a profile it reports as existing (for a cluster with none, validate it yourself — `SOUL.md` §6 step 1), then:
