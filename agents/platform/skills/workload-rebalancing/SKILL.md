...
Resolve each cluster's profile name first with the `resolve_cluster_agent(cluster_name, location, project_id)` tool, and assign a card only to a profile it reports as existing, then:
