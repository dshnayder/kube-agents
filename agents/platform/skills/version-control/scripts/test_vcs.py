...
class BrokerCallTest(unittest.TestCase):
    ...
    def test_a_disabled_broker_is_reported_as_disabled(self):
        with mock.patch.dict(
            os.environ, {"CREDENTIAL_PROXY_URL": "http://127.0.0.1:8080"}
        ), mock.patch.object(
            vcs.credential_proxy_client,
            "vcs_call",
            side_effect=vcs.credential_proxy_client.WorkspaceUnavailable("VCS_DISABLED"),
        ):
            with self.assertRaises(vcs.VcsError) as caught:
                vcs.call("clone", {})
        self.assertIn("turned off", str(caught.exception))
