...
class WorkspaceGitPathTest(unittest.TestCase):
    ...
    def test_the_forge_cli_runs_in_the_broker_root_rather_than_refusing_it(self):
        ...
        `execute_forge_cli` chooses the VCS root as its cwd on purpose, so
        ...
        publish kept working because they go through `execute_vcs_git`, while
        every proposal and issue verb 500ed before `gh` was launched.
        """
        executor = self.executor(CREDENTIAL_PROXY_VCS="1")
        result = executor.execute_forge_cli(["/bin/echo", "hello"])
        self.assertEqual(0, result.returncode)
        self.assertEqual("hello", result.stdout.strip())

    def test_the_two_broker_git_doors_do_not_share_a_root_or_a_subcommand_list(self):
        """Separately enabled features, so separately contained.

        A shared root would let one broker's cleanup reach the other's trees,
        and a shared subcommand list would hand each path the other's reach —
        `bundle` and `init` to the tree-authoring path, `add` and `commit` to
        the one that deliberately never checks anything out.
        """
        executor = self.executor(CREDENTIAL_PROXY_VCS="1")
        self.assertNotEqual(executor.content_workspace_root, executor.vcs_root)
        self.assertFalse(
            credential_proxy._within(executor.vcs_root, executor.content_workspace_root)
        )
        self.assertFalse(
            credential_proxy._within(executor.content_workspace_root, executor.vcs_root)
        )

        tree = executor.vcs_root / "repo"
        tree.mkdir(parents=True)
        subprocess.run(
            ["git", "init", "--quiet"], cwd=tree, check=True, capture_output=True
        )
        # Each door refuses what the other issues.
        with self.assertRaises(ValueError):
            executor.execute_vcs_git(["git", "commit", "-m", "x"], tree)
        with self.assertRaises(ValueError):
            executor.execute_workspace_git(["git", "ls-remote", "origin"], self.tree(executor))
        # Paired ordinary use: the VCS door runs the subcommand it does issue.
        done = executor.execute_vcs_git(["git", "rev-parse", "--git-dir"], tree)
        self.assertEqual(0, done.returncode)

    def test_the_vcs_door_refuses_a_working_directory_redirect(self):
        executor = self.executor(CREDENTIAL_PROXY_VCS="1")
        tree = executor.vcs_root / "repo"
        tree.mkdir(parents=True)
        with self.assertRaises(ValueError):
            executor.execute_vcs_git(["git", "-C", "/", "rev-parse"], tree)

    def test_the_vcs_door_is_unreachable_while_version_control_is_off(self):
        executor = self.executor()
        self.assertIsNone(executor.vcs_root)
        with self.assertRaises(RuntimeError):
            executor.execute_vcs_git(["git", "rev-parse"], Path("/"))
        with self.assertRaises(RuntimeError):
            executor.execute_forge_cli(["gh", "api", "/rate_limit"])

    def test_the_allowlist_is_still_the_four_names_the_shims_cover(self):
        """`/v1/vcs/*` must not add a fifth.

        The broker's git and `gh` both resolve through the same four-name
        allowlist the shims use, so version control already works with it
        exactly as it stands. An entry added "for the abstraction" would buy
        nothing while widening the one route that hands the model a
        credentialed process of its own choosing. A second version-control
        system arrives as a forge class in `vcs_broker`, not as a name here.
        """
        self.assertEqual(
...
class WorkspaceRouteTest(unittest.TestCase):
    ...
    def test_containment_root_has_only_the_callers_it_is_meant_to(self):
        ...
        because of who passes it, not because of what it does. The two here are
        the broker's own git, which takes the root as a parameter so that the
        content-workspace and version-control paths cannot reach each other's
        trees, and `gh`, which runs in the VCS root itself.
        """
...
class VcsRouteTest(unittest.TestCase):
    ...
    def test_an_underscored_verb_reaches_the_hyphenated_route(self):
        ...
        handler._read_json_body = lambda max_bytes=None: {"repository": "o/r"}
        handler._handle_vcs_post()
    ...
    def test_the_body_limit_is_the_bundle_ceiling_not_the_generic_one(self):
        ...
        handler._read_json_body = lambda max_bytes=None: (
            limits.append(max_bytes) or {"repository": "o/r"}
        )
        handler._handle_vcs_post()
    ...
    def test_the_wired_limit_clears_base64_expansion_of_the_ceiling(self):
        """What `serve()` actually sets, not what this test class set up."""
        with mock.patch.dict(
            os.environ, {"CREDENTIAL_PROXY_MAX_BUNDLE_BYTES": str(8 << 20)}
        ):
            ceiling = vcs_broker.max_bundle_bytes()
            wired = ceiling * 4 // 3 + credential_proxy.VCS_REQUEST_HEADROOM_BYTES
    ...
    def test_a_broker_refusal_keeps_its_status_and_code(self):
        broker = self._broker()
        broker.publish.side_effect = workspace_paths.WorkspaceError(
