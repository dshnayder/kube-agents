...
- **SOP Repository Authentication Bootstrap Gate**: Before executing `git clone` or repository operations, refresh your GitHub App installation token:
  - **Before cloning for the first time** (the `repo` directory does not exist):
    Extract the repository name (in `owner/repo` format, e.g. `your-org/your-repo`) from `/opt/data/SETTINGS.md` and execute the token refresher:
    ```bash
