...
next `upgrade.sh` regenerates from that file and is refused without the key.
...
report written before the interview decided them (a run that failed early) says so: `gvisor_enabled`
is `null` and the other two are empty, rather than restating a default the run never applied.
