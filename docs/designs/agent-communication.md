...
| 3    | ~~A narrow per-user default-cluster preference.~~ **Done** — via general per-user memory rather than a scoped router tool.     | `kage_memory` on the `default` profile, with a bank per gateway `user_id`. One sub-question resolved the hard way: in a **shared thread** `user_id` is whoever opened the session, because `build_session_key()` omits the participant id there unless `thread_sessions_per_user` is set — so a thread's participants would share one bank. The provider detects that case and disables the personal bank rather than crossing users, which makes per-user memory DM-only by design. |
