...
then left the lease behind. It works over the version-control verbs now, and the verbs hand each
container one copy per repository under its own scratch root. Nothing is shared, so there is nothing
to lease and no `assert_lease_owner` for `submit` to call: `--workspace` and `--lease` are both
retired flags that warn and are ignored. `prepare --branch <name>` brings the repository down and
