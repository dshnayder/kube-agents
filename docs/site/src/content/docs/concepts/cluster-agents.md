...
1. The Platform Agent resolves the cluster's profile name with its `resolve_cluster_agent` tool, which also says whether that profile exists, and files a card: `kanban_create(assignee="<profile>", body="<namespace/workload, symptom, time window>")`.
