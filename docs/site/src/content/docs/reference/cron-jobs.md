...
| `deliver`          | string          | Where the run's outcome goes. `"all"` sends it to the configured target; `"chat"` hands the report to the Chat Agent, who posts it and can answer a follow-up about it; `"local"` resolves to no target at all and drops it. No watchdog uses `"local"`: a failed run must stay audible.                    |
