...
- **Minty token exhaustion.** GitHub App installation tokens are 1-hour scoped. Minty caches and re-mints as needed; if the token refresher fails, `submit-suggestion` will surface it and route through the recovery ladder. `github-repo-watcher` has no ladder to route through — it is a script, not a model run — so a broker that refuses surfaces in the room as a reason code: `FORGE_UNAUTHENTICATED` when the token was minted and the forge would not accept it, `BROKER_UNREACHABLE` or `SANDBOX_UNREACHABLE` when the call never got that far, and `FORGE_NOT_FOUND`, `FORGE_RATE_LIMITED` or `REPO_UNREACHABLE` for the forge's own answers. Which of those it is distinguishes a credential the forge rejected from a transport that is down — a distinction the operator needs, because only the first is about the token. Why the forge rejected it is not carried with the code: it is rendered into a chat room, so it stays detail-free. The refusal itself is logged by the `envoy-credential-proxy` container, which runs the refresh helper and records its stderr with token-shaped substrings redacted; read that, or Minty's own logs.
