...
| `docs/designs/context-scoped-capabilities.md` | Feature design | Parked design for a capability-scoping layer that decides per turn which tools and skills the model sees in full, by name, or by search; the A/B results, a semantic-ranker estimate, why scoping does not pay at today's catalogue size, and what reopens it. | Experiment results, decision, cheaper fixes, revisit triggers, working-set policy, ranker, scoping record, alternatives | Parked; the prototype in the tree is inert unless enabled (banner in file) |
