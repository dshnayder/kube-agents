...
| `docs/designs/context-scoped-capabilities.md` | Feature design | Proposal for a harness-independent capability-scoping layer that decides per turn which tools and skills the model sees in full, by name, or by search; the measurement phase gates enforcement. | Catalogue metadata, task scope, working-set policy, ranker, scoping record, alternatives, A/B plan and its first run, Hermes mapping | Proposal for review; the prototype in the tree is inert unless enabled (banner in file) |
