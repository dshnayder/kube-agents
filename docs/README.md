...
├── bench/                                         devops-bench evaluation harness README; experiments/ holds A/B records
...
| `docs/designs/context-scoped-capabilities.md` | Feature design | Parked design for a capability-scoping layer that decides per turn which tools and skills the model sees in full, by name, or by search; the A/B results, a semantic-ranker estimate, why skill scoping does not pay at today's catalogue size, why the tools in a request leave nothing to scope, and what reopens it. | Experiment results, decision, cheaper fixes, revisit triggers, working-set policy, ranker, scoping record, alternatives | Parked; the prototype in the tree is inert unless enabled (banner in file) |
...
| `bench/experiments/capability-scope-ab/README.md` | Experiment | Method and code for the capability-scoping A/B behind `docs/designs/context-scoped-capabilities.md`: arms, rungs, probes, metrics, how to rerun it. | Arms, rungs, `scenarios.json`, `run_matrix.sh`, `analyze.py`, `results/`, limits | Experiment record; the design document reads its `results/` |
| `bench/experiments/capability-scope-ab/results/gemini-3.1-pro.md` | Experiment record | Scored table and arm comparisons for the capability-scoping A/B on Gemini 3.1 Pro, written by `make_results.sh`. | Selection, cost, intervals, p-values | Generated; do not hand-edit |
| `bench/experiments/capability-scope-ab/results/gemini-3.5-flash.md` | Experiment record | The same tables for the Gemini 3.5 Flash repeat. | Selection, cost, intervals, p-values | Generated; do not hand-edit |
