...
| `docs/designs/gcp-api-relay.md` | Feature design | Design only, not implemented: a read-only Google Cloud REST relay (`GET /v1/gcp/<host>/<path>`) in the credential broker, so a collector in the shell sandbox can read Cloud Monitoring time series without holding a token. Written for the `fleet-audit-collectors` branch. | Host and path allowlist, refused token-minting hosts, why scope narrowing is unavailable on GKE, `ApiSession` client, tests and live validation | Design of record for the relay; nothing shipped |
