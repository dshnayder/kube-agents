...
| `docs/designs/version-control-abstraction.md` | Feature design | The third answer to getting a sandboxed agent to a repository: history moves as a git bundle in both directions and is never checked out on the credential side, while the sandbox reads it with its own off-PATH git. Covers what the shared volume and content passing each give up, the `/v1/vcs/*` routes, the two gits, where the verb names come from, and what replacing `gh` still needs. | Bundles both ways, `/v1/vcs/*`, `/opt/vcs/libexec/git`, researched concept vocabulary, forge allowlist, `gh` removal unfinished | Implemented behind `CREDENTIAL_PROXY_VCS`; `inspect-repository` is still the default door |
