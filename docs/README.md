...
| `docs/designs/version-control-abstraction.md` | Feature design | Version control and issue tracking as forge-neutral verbs, so an install can point at GitLab or Bitbucket rather than only GitHub. Covers the customer requirement, why only remote operations are abstracted while local ones stay native git, the researched verb vocabulary, what a new forge module has to supply, the `/v1/vcs/*` routes and bundle transport, and the A/B/C experiment against the two existing access designs. | Forge modules and the `Forge` interface, remote-abstracted/local-native split, bundles both ways, `/v1/vcs/*`, researched concept vocabulary, A/B/C results | Implemented for GitHub; on by default via `shellSandbox.versionControl`; GitLab and Bitbucket registered but refusing; plugin packaging deferred |
| `docs/designs/gitlab-forge.md` | Feature design | GitLab as the first forge that is not GitHub, filling in the `_StubForge` that registers `gitlab.com` and refuses. Covers what the second forge finds wrong with the seam — git's credential has no place on the `Forge` interface, and the `api` callable's own signature is `gh` argv — plus the group-access-token credential model, self-managed hosts as configuration rather than a second module, and the GitLab-shaped translation traps (`iid` not `id`, system notes, `opened` not `open`). | `Forge.git_config`, neutral `api` signature, `CliTransport`/`HttpTransport`, `build_forges(config)`, `allowed_paths`, five leaks not four | Proposed; nothing implemented; endpoint details marked live-verify; needs a GitLab to validate against |
