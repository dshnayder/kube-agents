...
| `docs/designs/gitlab-forge.md` | Feature design | GitLab as the second forge, and what a second forge settles in the shared contract: the credential as one object the forge chooses, a request that is not `gh` argv, a transport that is not assumed to be a CLI, one package serving many hosts, and the package layout that keeps a forge's name inside its own directory. Also the three-PR delivery order and where a GitLab gets stood up to validate it. | `Credential` strategy, `Transport` protocol, `providers/` layout, `AVAILABLE`/`for_config`, import-boundary test, omnibus GitLab CE | Proposed; NOT implemented, and neither is the seam it fills in (banner in file) |
...
| `docs/designs/version-control-abstraction.md` | Feature design | Reaching a forge from a credential-isolated agent: a broker that owns the verbs and the clone, forges behind a neutral interface, and git reduced to a shim. Carries the A/B/C experiment that chose the shape and the honest limits of what it buys. | Broker verbs, `Forge` interface, git shim, scratch clones, the experiment, what it does not fix | Design of record; nothing in it is on `main` yet (banner in file) |
