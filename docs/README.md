...
| `docs/designs/eval-baseline-storage.md` | Feature design | Where the eval scorer's results are stored and how a baseline is established, compared against and reset: the append-only JSONL record, the five-component version key, computed admission, the local and GCS backends, rung 6's judged comparison, and how a quality-over-time dashboard reads the same data. | Append-only JSONL, version key, computed admission, `objectCreator` immutability, rung 6 margin, BigQuery external table | Design of record; partially implemented — GCS backend defaults off, bucket and dashboard do not exist (banner in file) |
