...
| `bench/baselines/README.md` | Reference | The checked-in baseline store: the per-case JSON format, the five-component version key a record is filed under, how admission is computed from screening evidence rather than declared, and what invalidates a record. | Version key, admission bar, staleness | Developers changing the agent, the judge, the verifiers, or the fleet |
