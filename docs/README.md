...
| `docs/designs/predictive-operations.md` | Feature design | Proposes a third operating mode after reactive and proactive: the agent acts on a forecast threshold breach — a consumed quantity forecast against a declared limit — with TimesFM 2.5 as a credential-free in-cluster forecaster, the finding shape and lead-time tiers, the compute-then-judge split, the backtest gate against `predict_linear`, and the replay fixture the first eval case needs. | Reactive / proactive / predictive, forecast threshold breach, TimesFM 2.5 licence and footprint, GCP API relay as the metrics source, `forecast-breach` inject, hit-and-miss record | Proposal for review; nothing implemented — the Scope section names what an install already has |
...
| Path | Category | Purpose and summary | Key topics | Audience / notes |
| --- | --- | --- | --- | --- |
| `bench/experiments/timesfm-backtest/README.md` | Experiment | Method and code for the TimesFM day-ahead backtest behind `docs/designs/predictive-operations.md`: corpus, arms, baselines, metrics, how to rerun it. | `collect.py`, forecaster and harness images, `cluster.sh`, `results/` | Experiment record; the design document reads its `results/` |
| `bench/experiments/timesfm-backtest/results/summary.md` | Experiment record | Scored tables for the TimesFM backtest, written by `harness/analyze.py`. | MASE, WQL, coverage, peak and new-high scores, latency | Generated; do not hand-edit |
| `bench/experiments/timesfm-backtest/RESULTS.md` | Experiment record | Go/no-go reading of the TimesFM backtest for stakeholders: how often a day-ahead forecast lands within −10%/+5% of the actual value. | Point-by-point accuracy per series type, against no-model rules and by lead time; threshold warnings; recommendation | Experiment record; the design document links it |
| `bench/experiments/timesfm-backtest/results/decision.md` | Experiment record | Every table behind `RESULTS.md`, written by `harness/decide.py`. | Warning rules per quantile, threshold, peak kind and series type; daily-peak forecasts | Generated; do not hand-edit |
| `bench/experiments/timesfm-backtest/results/pointwise.md` | Experiment record | Point-by-point accuracy tables behind `RESULTS.md`, written by `harness/pointwise.py`. | Share within, above and below the band per series type, method, lead time and quantile | Generated; do not hand-edit |
| `bench/experiments/timesfm-backtest/results/horizon.md` | Experiment record | 8-hour against 24-hour TimesFM forecasts on the same clock hours, written by `harness/horizon.py`. | Share within, above and below the band per series type and window | Generated; do not hand-edit |
| `bench/experiments/timesfm-backtest/results/window.md` | Experiment record | Largest overshoot and undershoot of 8-hour against 24-hour TimesFM forecasts, written by `harness/window.py`. | Typical and bad-day extremes per series type, with 24 hours and 7 days of history | Generated; do not hand-edit |
| `bench/experiments/timesfm-backtest/results/window-staging.md` | Experiment record | The same 8-hour against 24-hour extremes on a busy staging cluster, written by `harness/window.py`. | Typical and bad-day extremes per series type, with 24 hours and 7 days of history | Generated; do not hand-edit |
| `bench/experiments/timesfm-backtest/results/timing.md` | Experiment record | How long one TimesFM forecaster takes per request, written by `harness/timing.py`. | Server and wall seconds by batch size and history length | Generated; do not hand-edit |
